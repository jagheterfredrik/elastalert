...
def assert_alerts(ea_inst, calls):
    ...
    assert ea_inst.rules['anytest']['alert'][0].alert.call_count == len(calls)
    for call_num, call_args in enumerate(ea_inst.rules['anytest']['alert'][0].alert.call_args_list):
...
def test_init_rule(ea):
    ...
    ea.rules['anytest']['starttime'] = '2014-01-02T00:11:22'
    ea.rules['anytest']['processed_hits'] = ['abcdefg']
    ...
    for prop in ['starttime', 'agg_matches', 'current_aggregate_id', 'processed_hits', 'minimum_starttime']:
        assert new_rule[prop] == ea.rules['anytest'][prop]
...
def test_query(ea):
    ...
    ea.run_query(ea.rules['anytest'], START, END)
    ea.current_es.search.assert_called_with(body={'query': {'filtered': {'filter': {'bool': {'must': [{'range': {'@timestamp': {'lte': END_TIMESTAMP, 'gt': START_TIMESTAMP}}}]}}}}, 'sort': [{'@timestamp': {'order': 'asc'}}]}, index='idx', _source_include=['@timestamp'], ignore_unavailable=True, size=ea.rules['anytest']['max_query_size'])


def test_query_with_fields(ea):
    ea.rules['anytest']['_source_enabled'] = False
    ea.current_es.search.return_value = {'hits': {'hits': []}}
    ea.run_query(ea.rules['anytest'], START, END)
    ea.current_es.search.assert_called_with(body={'query': {'filtered': {'filter': {'bool': {'must': [{'range': {'@timestamp': {'lte': END_TIMESTAMP, 'gt': START_TIMESTAMP}}}]}}}}, 'sort': [{'@timestamp': {'order': 'asc'}}], 'fields': ['@timestamp']}, index='idx', ignore_unavailable=True, size=ea.rules['anytest']['max_query_size'])


def test_query_with_unix(ea):
    ea.rules['anytest']['timestamp_type'] = 'unix'
    ea.rules['anytest']['dt_to_ts'] = dt_to_unix
    ea.current_es.search.return_value = {'hits': {'hits': []}}
    ea.run_query(ea.rules['anytest'], START, END)
    start_unix = dt_to_unix(START)
    end_unix = dt_to_unix(END)
    ea.current_es.search.assert_called_with(body={'query': {'filtered': {'filter': {'bool': {'must': [{'range': {'@timestamp': {'lte': end_unix, 'gt': start_unix}}}]}}}}, 'sort': [{'@timestamp': {'order': 'asc'}}]}, index='idx', _source_include=['@timestamp'], ignore_unavailable=True, size=ea.rules['anytest']['max_query_size'])


def test_query_with_unixms(ea):
    ea.rules['anytest']['timestamp_type'] = 'unixms'
    ea.rules['anytest']['dt_to_ts'] = dt_to_unixms
    ea.current_es.search.return_value = {'hits': {'hits': []}}
    ea.run_query(ea.rules['anytest'], START, END)
...
def test_no_hits(ea):
    ea.current_es.search.return_value = {'hits': {'hits': []}}
    ea.run_query(ea.rules['anytest'], START, END)
    assert ea.rules['anytest']['type'].add_data.call_count == 0


def test_no_terms_hits(ea):
    ea.rules['anytest']['use_terms_query'] = True
    ea.rules['anytest']['query_key'] = 'QWERTY'
    ea.rules['anytest']['doc_type'] = 'uiop'
    ea.current_es.search.return_value = {'hits': {'hits': []}}
    ea.run_query(ea.rules['anytest'], START, END)
    assert ea.rules['anytest']['type'].add_terms_data.call_count == 0
...
def test_some_hits(ea):
    ...
    ea.run_query(ea.rules['anytest'], START, END)
    assert ea.rules['anytest']['type'].add_data.call_count == 1
    ea.rules['anytest']['type'].add_data.assert_called_with([x['_source'] for x in hits_dt['hits']['hits']])


def test_some_hits_unix(ea):
    ea.rules['anytest']['timestamp_type'] = 'unix'
    ea.rules['anytest']['dt_to_ts'] = dt_to_unix
    ea.rules['anytest']['ts_to_dt'] = unix_to_dt
...
def test_duplicate_timestamps(ea):
    ...
    ea.run_query(ea.rules['anytest'], START, ts_to_dt('2014-01-01T00:00:00Z'))

    assert len(ea.rules['anytest']['type'].add_data.call_args_list[0][0][0]) == 3
    assert ea.rules['anytest']['type'].add_data.call_count == 1

    # Run the query again, duplicates will be removed and not added
    ea.run_query(ea.rules['anytest'], ts_to_dt('2014-01-01T00:00:00Z'), END)
    assert ea.rules['anytest']['type'].add_data.call_count == 1
...
def test_match(ea):
    hits = generate_hits([START_TIMESTAMP, END_TIMESTAMP])
    ea.current_es.search.return_value = hits
    ea.rules['anytest']['type'].matches = [{'@timestamp': END}]
    with mock.patch('elastalert.util.Elasticsearch'):
        ea.run_rule(ea.rules['anytest'], END, START)

    ea.rules['anytest']['alert'][0].alert.called_with({'@timestamp': END_TIMESTAMP})
    assert ea.rules['anytest']['alert'][0].alert.call_count == 1
...
def test_run_rule_calls_garbage_collect(ea):
    ...
    with contextlib.nested(mock.patch.object(ea.rules['anytest']['type'], 'garbage_collect'),
                           mock.patch.object(ea, 'run_query')) as (mock_gc, mock_get_hits):
        ea.run_rule(ea.rules['anytest'], ts_to_dt(end_time), ts_to_dt(start_time))
...
def run_rule_query_exception(ea, mock_es):
    ...
    assert ea.rules['anytest'].get('starttime') == START
    assert ea.rules['anytest']['type'].add_data.call_count == 0
    assert ea.rules['anytest']['type'].garbage_collect.call_count == 0
    assert ea.rules['anytest']['type'].add_count_data.call_count == 0
...
def test_query_exception_count_query(ea):
    ea.rules['anytest']['use_count_query'] = True
    ea.rules['anytest']['doc_type'] = 'blahblahblahblah'
    mock_es = mock.Mock()
...
def test_match_with_module(ea):
    mod = BaseEnhancement(ea.rules['anytest'])
    mod.process = mock.Mock()
    ea.rules['anytest']['match_enhancements'] = [mod]
    test_match(ea)
...
def test_agg(ea):
    ...
    with mock.patch('elastalert.util.Elasticsearch'):
        ...
        ea.rules['anytest']['aggregation'] = datetime.timedelta(minutes=10)
        ea.rules['anytest']['type'].matches = [{'@timestamp': h} for h in hits_timestamps]
        ea.run_rule(ea.rules['anytest'], END, START)
...
def test_agg_cron(ea):
    ...
    with mock.patch('elastalert.util.Elasticsearch'):
        with mock.patch('elastalert.elastalert.croniter.get_next') as mock_ts:
            ...
            ea.rules['anytest']['aggregation'] = {'schedule': '*/5 * * * *'}
            ea.rules['anytest']['type'].matches = [{'@timestamp': h} for h in hits_timestamps]
            ea.run_rule(ea.rules['anytest'], END, START)
...
def test_agg_no_writeback_connectivity(ea):
    ...
    ea.current_es.search.return_value = hits
    ea.rules['anytest']['aggregation'] = datetime.timedelta(minutes=10)
    ea.rules['anytest']['type'].matches = [{'@timestamp': hit1},
                                           {'@timestamp': hit2},
                                           {'@timestamp': hit3}]
    ...
    assert ea.rules['anytest']['agg_matches'] == [{'@timestamp': hit1},
                                                  {'@timestamp': hit2},
                                                  {'@timestamp': hit3}]
    ...
    ea.add_aggregated_alert.assert_any_call({'@timestamp': hit1}, ea.rules['anytest'])
    ea.add_aggregated_alert.assert_any_call({'@timestamp': hit2}, ea.rules['anytest'])
    ea.add_aggregated_alert.assert_any_call({'@timestamp': hit3}, ea.rules['anytest'])
...
def test_silence(ea):
    ...
    ea.rules['anytest']['type'].matches = match
    with mock.patch('elastalert.util.Elasticsearch'):
        ea.run_rule(ea.rules['anytest'], END, START)
    assert ea.rules['anytest']['alert'][0].alert.call_count == 0
...
def test_compound_query_key(ea):
    ea.rules['anytest']['query_key'] = 'this,that,those'
    ea.rules['anytest']['compound_query_key'] = ['this', 'that', 'those']
    hits = generate_hits([START_TIMESTAMP, END_TIMESTAMP], this='abc', that=u'☃', those=4)
    ea.current_es.search.return_value = hits
    ea.run_query(ea.rules['anytest'], START, END)
    call_args = ea.rules['anytest']['type'].add_data.call_args_list[0]
...
def test_silence_query_key(ea):
    ...
    ea.rules['anytest']['type'].matches = match
    ea.rules['anytest']['query_key'] = 'username'
    with mock.patch('elastalert.util.Elasticsearch'):
        ea.run_rule(ea.rules['anytest'], END, START)
    assert ea.rules['anytest']['alert'][0].alert.call_count == 0
...
def test_realert(ea):
    ...
    ea.current_es.search.return_value = hits
    with mock.patch('elastalert.util.Elasticsearch'):
        ea.rules['anytest']['realert'] = datetime.timedelta(seconds=50)
        ea.rules['anytest']['type'].matches = matches
        ea.run_rule(ea.rules['anytest'], END, START)
        assert ea.rules['anytest']['alert'][0].alert.call_count == 1
    ...
    with mock.patch('elastalert.elastalert.ts_now') as mock_ts:
        with mock.patch('elastalert.util.Elasticsearch'):
            ...
            ea.rules['anytest']['type'].matches = matches
            ea.run_rule(ea.rules['anytest'], END, START)
            assert ea.rules['anytest']['alert'][0].alert.call_count == 2


def test_realert_with_query_key(ea):
    ea.rules['anytest']['query_key'] = 'username'
    ea.rules['anytest']['realert'] = datetime.timedelta(minutes=10)
    ...
    with mock.patch('elastalert.util.Elasticsearch'):
        ea.run_rule(ea.rules['anytest'], END, START)
    assert ea.rules['anytest']['alert'][0].alert.call_count == 3
    ...
    with mock.patch('elastalert.util.Elasticsearch'):
        ea.run_rule(ea.rules['anytest'], END, START)
    assert ea.rules['anytest']['alert'][0].alert.call_count == 4


def test_realert_with_nested_query_key(ea):
    ea.rules['anytest']['query_key'] = 'user.name'
    ea.rules['anytest']['realert'] = datetime.timedelta(minutes=10)
...
def test_count(ea):
    ea.rules['anytest']['use_count_query'] = True
    ea.rules['anytest']['doc_type'] = 'doctype'
    with mock.patch('elastalert.util.Elasticsearch'):
        ea.run_rule(ea.rules['anytest'], END, START)
...
def test_query_segmenting(ea):
    ...
    ea.rules['anytest']['buffer_time'] = datetime.timedelta(minutes=53)
    mock_es = mock.Mock()
    mock_es.search.side_effect = _duplicate_hits_generator([START_TIMESTAMP])
    with mock.patch('elastalert.util.Elasticsearch') as mock_es_init:
        mock_es_init.return_value = mock_es
        run_and_assert_segmented_queries(ea, START, END, ea.rules['anytest']['buffer_time'])
    ...
    ea.rules['anytest'].pop('use_count_query')
    ea.rules['anytest']['use_terms_query'] = True
    with mock.patch('elastalert.util.Elasticsearch'):
        run_and_assert_segmented_queries(ea, START, END, ea.run_every)
...
def test_set_starttime(ea):
    ...
    with mock.patch.object(ea, 'get_starttime') as mock_gs:
        ...
        ea.set_starttime(ea.rules['anytest'], end)
        assert mock_gs.call_count == 1
    assert ea.rules['anytest']['starttime'] == end - ea.buffer_time

    # Standard query, no starttime, rule specific buffer_time
    ea.rules['anytest'].pop('starttime')
    ea.rules['anytest']['buffer_time'] = datetime.timedelta(minutes=37)
    with mock.patch.object(ea, 'get_starttime') as mock_gs:
        mock_gs.return_value = None
        ea.set_starttime(ea.rules['anytest'], end)
        assert mock_gs.call_count == 1
    assert ea.rules['anytest']['starttime'] == end - datetime.timedelta(minutes=37)
    ea.rules['anytest'].pop('buffer_time')
    ...
    with mock.patch.object(ea, 'get_starttime') as mock_gs:
        ...
        ea.set_starttime(ea.rules['anytest'], end)
        assert mock_gs.call_count == 1
    assert ea.rules['anytest']['starttime'] == ts_to_dt('2014-10-10T00:00:00')

    # Standard query, no starttime, last run, assure buffer_time doesn't go past
    ea.rules['anytest'].pop('starttime')
    ea.rules['anytest']['buffer_time'] = datetime.timedelta(weeks=1000)
    with mock.patch.object(ea, 'get_starttime') as mock_gs:
    ...
    ea.set_starttime(ea.rules['anytest'], end)
    assert ea.rules['anytest']['starttime'] == ts_to_dt('2014-10-09T00:00:00')

    # Standard query, starttime
    ea.rules['anytest'].pop('buffer_time')
    ea.rules['anytest'].pop('minimum_starttime')
    with mock.patch.object(ea, 'get_starttime') as mock_gs:
        mock_gs.return_value = None
        ea.set_starttime(ea.rules['anytest'], end)
    ...
    ea.rules['anytest']['use_count_query'] = True
    ea.rules['anytest']['doc_type'] = 'blah'
    with mock.patch.object(ea, 'get_starttime') as mock_gs:
        mock_gs.return_value = None
        ea.set_starttime(ea.rules['anytest'], end)
        assert mock_gs.call_count == 0
    assert ea.rules['anytest']['starttime'] == end - ea.run_every

    # Count query, with previous endtime
    with mock.patch('elastalert.util.Elasticsearch'):
        ea.run_rule(ea.rules['anytest'], END, START)
    ea.set_starttime(ea.rules['anytest'], end)
    assert ea.rules['anytest']['starttime'] == END

    # buffer_time doesn't go past previous endtime
    ea.rules['anytest'].pop('use_count_query')
    ea.rules['anytest']['previous_endtime'] = end - ea.buffer_time * 2
    ea.set_starttime(ea.rules['anytest'], end)
    assert ea.rules['anytest']['starttime'] == ea.rules['anytest']['previous_endtime']


def test_kibana_dashboard(ea):
    match = {'@timestamp': '2014-10-11T00:00:00'}
    mock_es = mock.Mock()
    ea.rules['anytest']['use_kibana_dashboard'] = 'my dashboard'
    with mock.patch('elastalert.util.Elasticsearch') as mock_es_init:
        mock_es_init.return_value = mock_es
        ...
        with pytest.raises(EAException):
            ea.use_kibana_link(ea.rules['anytest'], match)
        ...
        url = ea.use_kibana_link(ea.rules['anytest'], match)
        ...
        ea.rules['anytest']['query_key'] = 'foobar'
        match['foobar'] = 'baz'
        url = ea.use_kibana_link(ea.rules['anytest'], match)
        ...
        # Compound query key
        ea.rules['anytest']['query_key'] = 'foo,bar'
        ea.rules['anytest']['compound_query_key'] = ['foo', 'bar']
        match['foo'] = 'cat'
        match['bar'] = 'dog'
...
def test_strf_index(ea):
    ...
    ea.rules['anytest']['use_strftime_index'] = True

    # Test formatting with times
    start = ts_to_dt('2015-01-02T12:34:45Z')
    end = ts_to_dt('2015-01-02T16:15:14Z')
    assert ea.get_index(ea.rules['anytest'], start, end) == 'logstash-2015.01.02'
    end = ts_to_dt('2015-01-03T01:02:03Z')
    assert ea.get_index(ea.rules['anytest'], start, end) == 'logstash-2015.01.02,logstash-2015.01.03'

    # Test formatting for wildcard
    assert ea.get_index(ea.rules['anytest']) == 'logstash-*'
    ea.rules['anytest']['index'] = 'logstash-%Y.%m'
    assert ea.get_index(ea.rules['anytest']) == 'logstash-*'
    ea.rules['anytest']['index'] = 'logstash-%Y.%m-stuff'
    assert ea.get_index(ea.rules['anytest']) == 'logstash-*-stuff'


def test_count_keys(ea):
    ea.rules['anytest']['timeframe'] = datetime.timedelta(minutes=60)
    ea.rules['anytest']['top_count_keys'] = ['this', 'that']
    ea.rules['anytest']['type'].matches = {'@timestamp': END}
    ea.rules['anytest']['doc_type'] = 'blah'
    ...
    counts = ea.get_top_counts(ea.rules['anytest'], START, END, ['this', 'that'])
...
def test_exponential_realert(ea):
    ea.rules['anytest']['exponential_realert'] = datetime.timedelta(days=1)  # 1 day ~ 10 * 2**13 seconds
    ea.rules['anytest']['realert'] = datetime.timedelta(seconds=10)
    ...
    for args in test_values:
        ea.silence_cache[ea.rules['anytest']['name']] = (args[1], args[2])
        next_alert, exponent = ea.next_alert_time(ea.rules['anytest'], ea.rules['anytest']['name'], args[0])
...
def test_notify_email(ea):
    mock_smtp = mock.Mock()
    ea.rules['anytest']['notify_email'] = ['foo@foo.foo', 'bar@bar.bar']
    with mock.patch('elastalert.elastalert.SMTP') as mock_smtp_f:
        mock_smtp_f.return_value = mock_smtp

        # Notify_email from rules, array
        ea.send_notification_email('omg', rule=ea.rules['anytest'])
        assert set(mock_smtp.sendmail.call_args_list[0][0][1]) == set(ea.rules['anytest']['notify_email'])

        # With ea.notify_email
        ea.notify_email = ['baz@baz.baz']
        ea.send_notification_email('omg', rule=ea.rules['anytest'])
        assert set(mock_smtp.sendmail.call_args_list[1][0][1]) == set(['baz@baz.baz'] + ea.rules['anytest']['notify_email'])

        # With ea.notify email but as single string
        ea.rules['anytest']['notify_email'] = 'foo@foo.foo'
        ea.send_notification_email('omg', rule=ea.rules['anytest'])
        assert set(mock_smtp.sendmail.call_args_list[2][0][1]) == set(['baz@baz.baz', 'foo@foo.foo'])

        # None from rule
        ea.rules['anytest'].pop('notify_email')
        ea.send_notification_email('omg', rule=ea.rules['anytest'])
